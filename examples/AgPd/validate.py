...
def get_differences(result, reference):
    ...
    for i in range(len(result_data)):
        ...
        atom_count.append(len(result_data[i]['coords']))
    ...
    print('Avg number of atoms per structure: {}'.format(np.sum(atom_count) / len(atom_count)))
...
def scatterplot(result_energy, reference_energy, system_type, force_diffs, quip_time):
    ...
    plt_color = {'fcc_Ag': 'y', 'bcc_Ag': 'navy', 'hcp_Ag': 'g', 'sc_Ag': 'm', 'slab_Ag': 'r',
                 'fcc_Pd': 'y', 'bcc_Pd': 'navy', 'hcp_Pd': 'g', 'sc_Pd': 'm', 'slab_Pd': 'r',
                 'fcc_AgPd': 'y', 'bcc_AgPd': 'navy', 'hcp_AgPd': 'g', 'sc_AgPd': 'm'}

    all_list = [['fcc_Ag', 'bcc_Ag', 'hcp_Ag', 'sc_Ag', 'slab_Ag'],
                ['fcc_Pd', 'bcc_Pd', 'hcp_Pd', 'sc_Pd', 'slab_Pd'],
    ...
    for ispec, sub_list in enumerate(all_list):
        ...
        ax[ispec].plot([-5.25, 0.5], [-5.25, 0.5], ':', color='k', linewidth=0.25)
        ...
        ax[ispec].set_xlim(-5.5, 1)
        ax[ispec].set_ylim(-5.5, 1)
