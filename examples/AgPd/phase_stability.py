...
if not plot_only:
    ...
    for csys in ['fcc', 'bcc', 'sc', 'hcp']:
        ...
        sys.stdout.flush()
        os.chdir(base_dir)
        cell = Structure.from_file(os.path.join(csys, 'CONTCAR'))
        for dv in np.arange(start=-0.6, stop=1.9, step=0.1):
            ...
            for file in os.listdir(base_dir):
                if file.startswith('AgPd.xml') or file.startswith('compress.dat'):
                    os.symlink(os.path.join('..', '..', file), file)

            os.system('quip atoms_filename=cell.xyz param_filename=AgPd.xml e > quip.result')
