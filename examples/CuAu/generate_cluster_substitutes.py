...
car_dir = os.path.expanduser('~/work/Aalto/GAP_data/Au/Hannes_data/clusters/vasp_outputs')
...
for car in cars:
    atoms = read(os.path.join(car_dir, car), index=':')
    for at in atoms:
        for _ in range(3):
            cluster = AseAtomsAdaptor().get_structure(at)
            for idx in random.sample(range(len(cluster.sites)), random.randint(1, len(cluster.sites)-10)):
                cluster.replace(idx, 'Cu')
            cluster.sort()
            ...
            for s in cluster.sites:
                if s.specie.name == 'Cu':
                    ...
                    site_properties['initial_moment'].append(-1.0)
            spin_cluster = Structure(lattice=cluster.lattice, species=cluster.species, coords=cluster.frac_coords,
                                     coords_are_cartesian=False, site_properties=site_properties)
            incar_set = MPStaticSet(spin_cluster)
            structure_name = '{} substituted Hannes cluster'.format(spin_cluster.composition.element_composition,
                                                                    str(spin_cluster.symbol_set[0]))
            meta = {'name': structure_name, 'date': datetime.datetime.now().strftime('%Y/%m/%d-%T')}
            kpt_set = Kpoints.gamma_automatic([1, 1, 1], shift=[0, 0, 0])
            static_wf = get_static_wf(structure=spin_cluster, struc_name=structure_name, vasp_input_set=incar_set,
